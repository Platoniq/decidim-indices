      ...
      def form(klass)
        Class.new do
          ...
          def from_params(params, extra_context = {})
            answers = []
            answers << params[:questionnaire][:responses].to_unsafe_h.map do |answer|
              choices = []
              choices << answer.last[:choices]&.map do |choice|
                choice = choice.last if choice.is_a?(Array)
                Decidim::Forms::AnswerChoiceForm.new(body: choice[:body],
                                                     custom_body: choice[:custom_body],
                                                     position: choice[:position],
                                                     answer_option_id: choice[:answer_option_id],
                                                     matrix_row_id: choice[:matrix_row_id])
              end

              Decidim::Forms::AnswerForm.new(question_id: answer.last[:question_id], body: answer.last[:body], choices: choices.compact.flatten)
            end
            c = context.merge(responses: answers.flatten)
            @klass.from_params(params).with_context(c.merge(extra_context))
          end
